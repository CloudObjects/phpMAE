var phpMAE = {
    sessionId : null,
    functionMatcher : /(?:\/\*\*((?:[\s\S](?!\/\*))*?)\*\/+\s*)?public\s+function\s+(\w+)\s*\((.*)\)/g,
    codeEditor : CodeMirror(document.getElementById('phpmae-source-editor'), {
    ...
    configEditor : CodeMirror(document.getElementById('phpmae-config-editor'), {
...
function updateParameters(functionName) {
    ...
    var parametersHtml = "";
    do {
        functionMatch = phpMAE.functionMatcher.exec(sourceCode);
...
$(function() {
    ...
    phpMAE.codeEditor.on("change", function(instance, change) {
